...
- **触发**：空中（风爆/星月跃起后的窗口内）按跳跃键即向前上方突进，不要求疾跑；每次跃起限一次，落地或下一次跃起重置
...
- **触发**：持有效果期间，**疾跑状态下按跳跃键**即向前上方突进，突进时整体重置下坠速度；疾跑中空中**不限次数**连发（每 10 刻冷却）；风爆弹跳后可**在空中按住疾跑键+前进键起跑**再按跳跃接续
- **等级**：一级水平约 2 格、抬高约 2 格；二级水平约 5 格、抬高约 3 格（`/effect give @s flying_enchant:flying 30 1`）
- **消耗**：每次 0.5 点饥饿（半点累积，两次扣 1 点；不绑定武器、无耐久消耗，创造豁免）
- **双持优先级**：同时拥有附魔与效果时，跃起后第一按优先消耗**附魔**次数，之后落到效果连发；风爆重置附魔状态后附魔重新优先
