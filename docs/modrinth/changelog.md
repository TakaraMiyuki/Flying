...
- Respects the same `/gamerule flyingDash` switch and sprint food threshold
...
- **除玩家以外的所有生物免疫**该效果
- 与附魔版共用 `/gamerule flyingDash` 开关与饱食度门槛
...
- Server admins can disable dashing server-wide with `/gamerule flyingDash false`
...
- 服务器管理员可用 `/gamerule flyingDash false` 全服禁用
